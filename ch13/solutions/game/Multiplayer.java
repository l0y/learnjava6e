    ...
    public void run() {
      ...
      try {
        ...
        while (keepListening) {
          ...
          while (startNewGame) {
            ...
            if (response != null && response.equals("OK")) {
              ...
              for (Hedge hedge : gameField.hedges) {
                writer.println(
                  "HEDGE " + hedge.getPositionX() + " " + hedge.getPositionY()
                );
              }
            } else {
            ...
            if (!disconnecting) {
              ...
              int myPlayAgain = JOptionPane.showConfirmDialog(
                gameField,
                message,
                "Play Again?",
        ...
        try {
          ...
          System.err.println(
            "Error closing client socket: " + closingException.getMessage()
          );
        }
      }
    }
    ...
    public void stopListening() {
      if (listener != null && !listener.isClosed()) {
        try {
          ...
          System.err.println(
            "Error disconnecting listener: " + ioe.getMessage()
          );
        }
      }
    }
    ...
    public void run() {
      try (Socket socket = new Socket(gameHost, gamePort)) {
        ...
        while (startNewGame) {
          ...
          writer.println("OK");
          // And now gather the trees and hedges to setup our field
          gameField.trees.clear();
          gameField.hedges.clear();
          response = reader.readLine();
          while (response.startsWith("TREE") || response.startsWith("HEDGE")) {
            // both obstacles send their x,y coordinates after the first space
            String[] parts = response.split(" ");
          ...
          while (keepPlaying) {
            ...
            switch (parts[0]) {
              ...
              default:
                System.err.println(
                  "Unexpected game command: " + response + ". Ignoring."
                );
            }
            if (disconnecting) {
          ...
          if (!disconnecting) {
            ...
            if (response != null && response.equals("PLAY_AGAIN")) {
              ...
              String message =
                gameField.getWinner() + " Would you like to play again?";
              int myPlayAgain = JOptionPane.showConfirmDialog(
                gameField,
                message,
                "Play Again?",
