...
public class Apple implements GamePiece {
  ...
  public void setSize(int size) {
    ...
    // length of the sides. Since we want a circle, we simply make our box
    // a square so we only need one length.
  ...
  public boolean isTouching(GamePiece otherPiece) {
    ...
    if (otherPiece instanceof Apple) {
      ...
      double otherRadius =
        otherApple.getDiameter() * Field.APPLE_SIZE_IN_PIXELS / 2;
    ...
    // Must be an obstacle (tree or hedge). Use our box intersection helper
    if (
      GameUtilities.doBoxesIntersect(boundingBox, otherPiece.getBoundingBox())
    ) {
      // We have a collision! Store it and report the detection.
      setCollided(otherPiece);
      return true;
    }
    return false;
